...
void convolute(Image* srcImage,Image* destImage,Matrix algorithm){
    ...
    for (int i = 0; i < NUM_THREADS; i++){
        ...
        // Create thread
        if (pthread_create(&threads[i], NULL, convolute_thread, &thread_data[i]) != 0){
...
int main(int argc,char** argv){
    ...
    printf("Processing %dx%d image (%d channels) with %d threads...\n", 
           srcImage.width, srcImage.height, srcImage.bpp, NUM_THREADS);
    ...
    t2=time(NULL);
    printf("Convolution took %ld seconds with %d threads\n",t2-t1,NUM_THREADS);
